...
const columnConfig = {
  ...
  roadMarkingL: {
    ...
    render: (value) => (
      <span
        className={`inline-block w-6 h-6 rounded-full text-center font-bold ${
...
const RoadConditionsTable = () => {
  ...
  const priorityColumns = ["linkId", "chainageFrom", "chainageTo"];
  const allColumns = data.length > 0 ? Object.keys(data[0]) : [];
  // const columns = allColumns.filter((col) => col !== 'id');
  const columns = [
    ...priorityColumns,
    ...allColumns.filter(
      (key) => !priorityColumns.includes(key) && key !== "id"
    ),
  ];
  if (loading) {
    return (
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 px-10">
      <h2 className="text-3xl font-bold text-gray-800 mb-6 text-center">
        Uploaded road condition data
      </h2>
